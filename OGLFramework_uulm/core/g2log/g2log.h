...
const int GL_DEBUG = 0, DEBUG = 1, INFO = 2, WARNING = 3, ERROR = 4, FATAL = 5;
...
#define G2_LOG_GL_DEBUG  g2::internal::LogMessage(__FILE__,__LINE__,__PRETTY_FUNCTION__,"OPENGL ")
...
#define G2_LOGF_GL_DEBUG     g2::internal::LogMessage(__FILE__, __LINE__, __PRETTY_FUNCTION__,"OPENGL ")
...
std::wostream& operator<<(std::wostream&, const std::string&);
...
namespace internal
{
  typedef const std::wstring& LogEntry;
...
struct FatalMessage
{
  ...
  FatalMessage(std::wstring message, FatalType type, int signal_id);
  ~FatalMessage(){}; 
...
class LogMessage
{
  public:
    ...
    std::wostringstream& messageStream(){return stream_;}
