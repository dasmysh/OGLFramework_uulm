...
std::wostream& operator<<(std::wostream& out, const std::string& str)
{
    ...
    std::wstring wideString(wideStringArray.data(), str.length());
    out << wideString;
    return out;
}
...
namespace {
   ...
   std::wstring g_first_unintialized_msg;
...
FatalMessage::FatalMessage(std::wstring message, FatalType type, int signal_id)
  : message_(message)
  , type_(type)
