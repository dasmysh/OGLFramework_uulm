...
{
    unsigned int r = strtoul(v.substr(1, 2).c_str(), NULL, 16);
    unsigned int g = strtoul(v.substr(3, 2).c_str(), NULL, 16);
    unsigned int b = strtoul(v.substr(5, 2).c_str(), NULL, 16);
...
{
    std::stringstream conv;
    unsigned int r = static_cast<unsigned int> (v.r * 255.0f);
    unsigned int g = static_cast<unsigned int> (v.g * 255.0f);
    unsigned int b = static_cast<unsigned int> (v.b * 255.0f);
