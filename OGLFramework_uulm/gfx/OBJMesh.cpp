...
void OBJMesh::loadMeshData(std::ifstream& file)
{
    ...
    while (file.good()) {
        ...
        } else if (boost::regex_match(currLine, lineMatch, reg_vn)) { // TODO: normalize
            normals.push_back(glm::vec3(boost::lexical_cast<float>(lineMatch[1].str()),
...
{
    LOG(WARNING) << L"Feature not implemented: " << feature;
}
